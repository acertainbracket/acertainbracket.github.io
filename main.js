...
const side = 256;
const configSelectors = ['#min-value', '#max-value', '#min-x', '#max-x', '#min-y', '#max-y', '#unit-t', '#equation'];
const functionSelectors = [
  '.user-function-symbol',
  '.user-function-input-signature',
  '.user-function-output-signature',
  '.user-function-inputs',
  '.user-function-equation',
  '.readonly-user-function-symbol',
];
const constantSelectors = [
  '.user-constant-symbol',
  '.user-constant-value',
];
...
const operationDict = {
  ...
  'Tuple': node => {
    if (node?.[1]?.[0] === 'Power') {
      const functionSymbol = node[1][1]?.[0] === "Subscript" ? `${node[1][1][1]}_${node[1][1][2]}`: node[1][1];
      if (!Object.hasOwn(objIterations, functionSymbol)) {
        objIterations[functionSymbol] = []
      }
      iterations.push({symbol: functionSymbol, amount: node[1][2][2], ...symbolsMap[functionSymbol]});
      objIterations[functionSymbol].push({symbol: functionSymbol, amount: node[1][2][2], ...symbolsMap[functionSymbol]});
      if (node?.[2]?.[0] === 'Tuple') {
        let inputs = node[2].slice(1).map((child, index) => symbolsMap[functionSymbol].inputSignature[index] === 'complex' ? `injection_map(${mj2gl(child)})` : mj2gl(child))
        return `iterate_${functionSymbol}_${node[1][2][2]}(${inputs.join(",")})`;
      }
      let input = symbolsMap[functionSymbol].inputSignature[0] === 'complex' ? `injection_map(${mj2gl(node[2])})` : mj2gl(node[2]);
...
function generateFunctions(functionDeclarations, allSignatures, definitionElements, equationComputeEngine) {
  ...
  ${structName} to_tuple(${structName.split("_").map((type, i) => `${type} arg${i}`).join(', ')}){
    ...
    .map(({equationElement, functionSymbol, variables, inputSignature, outputSignature}) => {
    const math = mj2gl(JSON.parse(equationElement.getValue('math-json')))
    console.log(functionSymbol);
    console.log(JSON.stringify(objIterations));
    return (`
  ...
  }` : '') + (Object.hasOwn(objIterations, functionSymbol) ? (objIterations[functionSymbol].map(({symbol, amount}) => `
...
function generateGLSL() {
  ...
  equationComputeEngine.latexDictionary = equationComputeEngine.latexDictionary.filter(a => a.parse !== "ImaginaryUnit");
  equationComputeEngine.latexDictionary = equationComputeEngine.latexDictionary.concat([
    iSymbol,
  ]);
  ...
  `;
  console.log(fragSrc.split("\n").map((line, index) => (index + 1).toString().padStart(3, " ") + line).join("\n"));
...
const createNewFunctionDefinitionElement = () => {
  ...
  clone.querySelector('.left-button').onclick = (e) => {
    e.target.closest('.user-function-definition').previousElementSibling?.before(e.target.closest('.user-function-definition'));
    setupElement();
  }
  clone.querySelector('.right-button').onclick = (e) => {
    e.target.closest('.user-function-definition').nextElementSibling?.after(e.target.closest('.user-function-definition'));
    setupElement();
  }
...
const createNewConstantDefinitionElement = () => {
  ...
  clone.querySelector('.left-button').onclick = (e) => {
    e.target.closest('.user-constant-definition').previousElementSibling?.before(e.target.closest('.user-constant-definition'));
  }
  clone.querySelector('.right-button').onclick = (e) => {
    e.target.closest('.user-constant-definition').nextElementSibling?.after(e.target.closest('.user-constant-definition'));
  }
  parentElement.appendChild(clone);
  document.querySelectorAll('math-field[readonly]').forEach(mf => mf.tabIndex = -1);
  return parentElement.lastElementChild;
...
document.querySelector('#save-button').onclick = () => {
  const entries = configSelectors.map(selector => [selector, document.querySelector(selector).value]);
  const settings = Object.fromEntries(entries);
  const functions = Array.from(document.querySelectorAll('.user-function-definition')).map(definitionElement => 
    Object.fromEntries(
      functionSelectors.map(selector => [selector, definitionElement.querySelector(selector).value])
    )
  );
  const constants = Array.from(document.querySelectorAll('.user-constant-definition')).map(definitionElement =>
    Object.fromEntries(
      constantSelectors.map(selector => [selector, definitionElement.querySelector(selector).value])
    )
  );
  const content = JSON.stringify({settings, functions, constants});
  const a = document.createElement("a");
  const file = new Blob([content], {type: 'text/plain'});
  a.href = URL.createObjectURL(file);
  a.download = 'equation-shader.txt';
  a.click();
};
  ...
  input.onchange = () => {
    ...
    reader.onload = readerEvent => {
      ...
      for (let id of configSelectors) {
        document.querySelector(id).value = data?.settings?.[id];
      }
      if (Array.isArray(data?.functions)) {
        for (let functionData of data?.functions) {
          const newDefinitionElement = createNewFunctionDefinitionElement();
          for (let functionSelector of functionSelectors) {
            newDefinitionElement.querySelector(functionSelector).value = functionData?.[functionSelector];
          }
        }
      }
      if (Array.isArray(data?.constants)) {
        for (let constantData of data?.constants) {
          const newDefinitionElement = createNewConstantDefinitionElement();
          for (let constantSelector of constantSelectors) {
            newDefinitionElement.querySelector(constantSelector).value = constantData?.[constantSelector];
          }
        }
      }
      animate();
